    ...
    <div >
        <div class="navbar-container">
            <div class='navbar-left'>
                ...
                <div class='navbar-board-link' v-on:click="openmodal('modalboard')">
              ...
              <div class="navbar-header">
                  <svg v-on:click='homepage()' style="top: 3.8px; left: 45%;" class="d-block" width="130" height="30" viewBox="0 0 885 272" alt="Trello">
                ...
                <div class="navbar-right">
                    <div v-on:click="openmodal('modalcreate')" class='navbar-icon'><font-awesome-icon icon="plus" class="icon" :style="{ color: '#fff' }"/></div>
                    <div v-on:click="openmodal('modalinformation')" class='navbar-icon'><font-awesome-icon icon="info-circle" class="icon" :style="{ color: '#fff' }"/></div>
                    <div v-on:click="openmodal('modalnotification')" class='navbar-icon'><font-awesome-icon icon="bell" class="icon" :style="{ color: '#fff' }"/></div>
                    <span v-on:click="openmodal('modalaccount')" class='navbar-user'>RL</span>
                </div>
            </div>
            <div v-bind:class=[modal]>
              <ModalMenu v-if="modal === 'modalmenu'"/>
              <ModalBoard v-if="modal === 'modalboard'"/>
              <ModalCreate v-if="modal === 'modalcreate'"/>
              <ModalInformation v-if="modal === 'modalinformation'"/>
              <ModalNotification v-if="modal === 'modalnotification'"/>
              <ModalAccount v-if="modal === 'modalaccount'"/>
            </div>
        </div>
</template>
...
.navbar-icon:hover, .navbar-user:hover{
  cursor: pointer;
}
.navbar-user{
  display: flex;
